...
* **Sync strategy compatibility:** The schema is designed to work with the described sync approach: clients will pull all tasks where `updated_at` is newer than their last sync or where `deleted_at` is set (indicating a deletion). Because we never truly delete rows immediately, even deletions show up as records with a `deleted_at` timestamp. The client can then reconcile these by removing them locally. On pushes/upserts, using the same primary keys (UUIDs) allows the server to do an UPSERT (insert or update) seamlessly. Conflict resolution favors the latest `updated_at` (last-write-wins for normal fields) and uses special rules for booleans and sets as noted in the architecture – our data model supports this by having the necessary fields (e.g., `is_completed` with a `completed_at` timestamp to decide conflicts within a small time window, and the separate TaskLabels table to merge label sets rather than a single field).
